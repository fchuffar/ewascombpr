...
rule ewas_combp:
    ...
    output: 
      html = "{prefix}/02_ewas_combp_{study_filename}.rds_{modelcall}_{model_formula}_{pval_thresh}.html"      ,           
    threads: 31
    shell:"""
export PATH="/summer/epistorage/opt/bin:$PATH"
export PATH="/summer/epistorage/miniconda3/bin:$PATH"
cd {wildcards.prefix}
echo "study_filename='{wildcards.study_filename}.rds'; model_func_name = '{wildcards.modelcall}' ; model_formula='{wildcards.model_formula}'; pval_thresh='{wildcards.pval_thresh}'; rmarkdown::render('{input.rmd_script}', output_file=paste0('02_ewas_combp_', study_filename, '_', model_func_name, '_', model_formula, '_', pval_thresh, '.html'))" | Rscript -
"""
